...
public class Filter {
    ...
    public static List<Tweet> containing(List<Tweet> tweets, List<String> words) {
          ...
          // Convert words list to a lowercase set for faster lookups
          ...
          for (Tweet tweet : tweets) {
              // Split the tweet text into words and convert them to lowercase
              String[] tweetWords = tweet.getText().toLowerCase().split("\\s+");
              
              // Check if any of the words in the tweet match the words we're looking for
              for (String tweetWord : tweetWords) {
                  if (lowerCaseWords.contains(tweetWord)) {
                      result.add(tweet);
                      break; // Stop checking this tweet if we found a match
                  }
              }
          }
